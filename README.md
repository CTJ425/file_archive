...
| 檔案 | 說明 |
|---|---|
| `sync_YYYY-MM-DD.log` | 每日逐檔文字 log（MOVE / KEEPOLD / DELETE / SUMMARY） |
| `summary_time.html` | 最近 200 次執行的摘要表（時間、搬移/刪除檔數與容量、跳過(過舊)/豁免保留筆數、狀態），用瀏覽器開即可看 |
...
- **保留一年**：只在 NAS 本機封存目錄 `$DST` 上刪除 mtime 超過 `RETENTION_DAYS` 的檔，
  以檔案**原始 mtime**為準（rsync `-a` 會保留原始時間）。**絕不對來源執行刪除**。
  保留基準在**開跑時定格**成固定時間點，不隨執行時間漂移。
- **來源上已超過保留期的檔**（例：A PC 積壓了 400 天前的舊檔，而 `RETENTION_DAYS=365`）
  由 `OVER_RETENTION_POLICY` 決定去向，三選一：

  | 值 | 行為 | 來源空間 | 資料保存 |
  |---|---|---|---|
  | `keep`（預設） | 搬到 NAS 並登記於 `$DST/.keepold.list`，**永久豁免**保留期刪除 | 釋放 | 完整保存（NAS 容量需自行留意） |
  | `archive` | 搬到 NAS，之後由保留期正常刪除 | 釋放 | 不保留（log 完整記錄 MOVE/DELETE） |
  | `skip` | 不搬也不刪，原地留在來源並記 `SKIPOLD` 警告 | 不釋放 | 完整保存（需人工處理） |

  搬移清單與保留刪除使用**同一個開跑時定格的保留基準時間點**，邊界保證一致 ——
  即使首次搬移大批積壓、rsync 跑數小時，也不會出現「掃描時未過期、保留階段已過期」
  而被同輪搬過去又刪掉的窗口。
- **豁免清單 `.keepold.list`**：刻意放在 `$DST` 內而非 log 目錄 —— 這份紀錄必須與封存
  資料同生共死，若放 log 目錄而被清掉，那些「刻意永久保留」的檔會在下一輪被靜默刪除。
  每次執行會自動剔除已不存在的項目避免無限增長；沒有任何豁免項時不留空檔案。
...
- **健康檢查**：來源可以是 CIFS 掛載點或**本機資料夾**。設 `REQUIRE_MOUNTPOINT=1`
  （CIFS 部署建議）時，未掛載即中止並記 ERROR —— 分享夾沒掛上時掛載點只是空目錄，
...
**對 `sync_archive.sh` 的預期行為**（應搬移／應留在來源／應記 `SKIPOLD` 各幾檔），
可直接與 log 的 `SUMMARY` 對帳：

- 今天之前的資料夾 → 應被搬移封存
- 今天的資料夾 → 應完全不動
- 超過保留期的檔 → 預設 `keep` 政策下應記 `KEEPOLD`，搬到 NAS 並永久豁免刪除
...
# 4) 掛載中止測試（需 conf 設 REQUIRE_MOUNTPOINT=1）：卸載 CIFS 後執行，
#    log 應出現 ERROR 且來源未被動
```
...
- **逐檔文字 log**：`logs/sync_YYYY-MM-DD.log`，格式如：
  ...
  KEEPOLD 524288  ./cam1/2024-01/clip_old.mp4        ← 超過保留期，仍封存並永久豁免刪除
  DELETE 1048576  /volume1/archive/2025/.../old.mp4
  2026-07-23 03:04:12  SUMMARY moved=3 movedBytes=... deleted=1 deletedBytes=... skippedOld=0 skippedBytes=... keptOld=1 keptBytes=... rsync_rc=0 dur=251s dry_run=0
  ```
...
| `OVER_RETENTION_POLICY` | `keep` | 來源上已超過保留期的檔：`keep`＝搬過去並永久豁免刪除／`archive`＝搬過去後由保留期刪除／`skip`＝留在來源不動 |
