...
| `summary_time.html` | 最近 200 次執行的摘要表（時間、搬移/刪除檔數與容量、狀態），用瀏覽器開即可看 |
...
- **逐檔文字 log**：`logs/sync_YYYY-MM-DD.log`，格式如：
  ...
  2026-07-23 03:04:12  SUMMARY moved=2 movedBytes=... deleted=1 deletedBytes=... rsync_rc=0 dur=251s dry_run=0
  ```
