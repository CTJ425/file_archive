...
### `ARCHIVE_BEFORE_DAYS_AGO` 的實際效果（實測對照）

以「今天 = 2026-07-24」、來源有 07-21～07-24 各一個檔為例：

| 設定 | 封存基準 | 搬走 | 留在來源 |
|---|---|---|---|
| `0`（預設） | `2026-07-24 00:00` | 07-23、07-22、07-21 | **今天** |
| `1` | `2026-07-23 00:00` | 07-22、07-21 | **今天＋昨天** |
| `3` | `2026-07-21 00:00` | 07-20 | **今天～前 3 天** |

`=1` 就是「多留昨天一天」，用在來源端有跨日補寫或後製流程、怕昨天的檔還沒完全寫完的情況。

- **剛好落在基準時刻的檔會被搬走**：判定是 `! -newermt`（不比基準新，即 `mtime <= 基準`），
  所以 mtime 正好是 `07-23 00:00:00` 的檔在 `=1` 時會被搬走。嚴格講「保留昨天」是
  「保留昨天 `00:00:00` **之後**」——要剛好卡在午夜零秒才會遇到，實務上碰不到。
- **只影響封存階段，與保留期無關**：刪除與否是 `RETENTION_DAYS` 在 `$DST` 上獨立決定的。
  調大這個值只是讓檔案晚幾天搬上 NAS，**不會**讓它們晚幾天被刪 —— 保留期以檔案自己的 mtime 計算。
- **填錯值的行為**：非數字（例如 `abc`）會讓 `date` 解析失敗，腳本直接以
  「無法計算基準日」中止並回傳 `1`，不會靜默亂搬；負數會被 GNU `date` 當成正數處理
  （`-1` 與 `1` 都得到同一個基準），**不會**產生未來日期而把今天正在寫入的檔掃走。
